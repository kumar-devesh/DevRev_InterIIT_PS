...
def prediction_stage(args, device, tokenizer, logger, debug=False):
    ...
    evaluation_stage(args, eval_examples, eval_features, device, model, logger)
...
def evaluation_stage(args, eval_examples, eval_features, device, model, logger, generate_prob_th=0.6,
    ...
    ''' !!! Edited !!! '''
    all_predictions = write_predictions(args, eval_examples, eval_features, all_results,
        args.n_best_size, args.max_answer_length,
        args.do_lower_case, output_prediction_file,
        output_nbest_file, output_null_odd_logs_file, args.verbose_logging, logger, args.output_prediction)
...
def training_stage(args, tokenizer, device, logger, debug=False):
    ...
    logger.info('The final model has been saved')
    logger.info('*** The Training Stage Ended ***')
    

def main():
    parser = argparse.ArgumentParser()

    parser.add_argument("--debug", default=False, help="use debug to test on a subset")
    ...
    if args.do_train:
        ...
        training_stage(args, tokenizer, device, logger, debug=args.debug)
