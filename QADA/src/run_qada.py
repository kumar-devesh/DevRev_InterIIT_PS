...
def prediction_stage(args, device, tokenizer, logger, debug=False):
    ...
    model_state_dict = torch.load(output_model_file+'.final')
    ...
    acc, f1 = evaluation_stage(args, eval_examples, eval_features, device, model, logger, wr=True)
...
def evaluate_acc_and_f1(predictions, raw_data, logger, threshold=-1, all_probs=None):
    ...
    for sample in raw_data:
        ...
        if not eval_threshold or (eval_threshold and all_probs[sample.qas_id] >= threshold):
            ground_truths = sample.orig_answer_text
...
def evaluation_stage(args, eval_examples, eval_features, device, model, logger, generate_prob_th=0.6,
        removed_feature_index=None, global_step=None, best_acc=None, best_f1=None, generate_label=False, wr=False):
    if not global_step:
    ...
    if global_step:
        ...
        output_null_odd_logs_file = os.path.join(args.output_dir, 'null_odds.json')
    else:
        output_prediction_file = os.path.join(args.output_dir, 'predictions.json')
        output_nbest_file = os.path.join(args.output_dir, 'nbest_predictions.json')
        output_null_odd_logs_file = os.path.join(args.output_dir, 'null_odds.json')
    
    all_predictions, all_probs, all_indices = write_predictions_qada(args, eval_examples, eval_features, all_results,
        args.n_best_size, args.max_answer_length,
        args.do_lower_case, output_prediction_file,
        output_nbest_file, output_null_odd_logs_file, args.verbose_logging, logger, args.output_prediction, wr=wr)
...
def main(debug=False):
    parser = argparse.ArgumentParser("QADA train file domain adaptation")
    parser.add_argument("--debug", default=False, help="subset training for debug")
    parser.add_argument("--version_2_with_negatives", default=False, help="use squadv2")
    parser.add_argument("--null_score_threshold", default=1.0, help="null score threshold")
    ...
    parser.add_argument("--output_model_file", default=None, type=str, required=False,
    ...
    parser.add_argument("--predict_file", default='./data/squad/dev-v2.0.json', type=str, help="SQuAD json for predictions/Dev. E.g., dev-v1.1.json or test-v1.1.json")
    
    parser.add_argument("--target_train_file", default='./data/squad/train-v2.0.json', type=str, help="Train file in target domain")
    parser.add_argument("--target_predict_file", default='./data/squad/dev-v2.0.json', type=str, help="Dev file in target domain")
    parser.add_argument("--source_train_file", default='./data/squad/train-v2.0.json', type=str, help="Train file in source domain")
    ...
    parser.add_argument("--warmup_proportion", default=0.1, type=float,
                        help="Proportion of training to perform linear learning rate warmup for. E.g., 0.1 = 10 percent "
                             "of training.")
    ...
    if args.do_adaptation:
        adaptation_stage(args, tokenizer, device, logger, debug=args.debug)

    if args.do_predict:
        prediction_stage(args, device, tokenizer, logger, debug=args.debug)
