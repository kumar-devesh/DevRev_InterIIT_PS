...
from evaluation_v2 import main as evaluate_squadv2
from config import *
from utils import *
...
def main():
    ...
    elif args.adaptation_method == 'smooth_can' and 'deberta' in args.bert_model:
        from modeling_debertav2_smooth_can import CustomizedDebertaV2ForQuestionAnswering as BertForQuestionAnswering

    elif args.adaptation_method == 'smooth_can' and 'bert' in args.bert_model:
    ...
    if 'deberta' in args.bert_model:
        from transformers import DebertaV2Tokenizer as BertTokenizer
    ...
    if args.do_train:
        ...
        if args.train_both and Path(cached_mrqa_train_features_file).is_file():
            ...
            print('Reading Synthetic examples...')
            mrqa_examples = read_squad_examples_synthetic(input_file=args.mrqa_train_file, is_training=True)
        ...
        if args.train_both:
            ...
            all_input_type = torch.tensor([1 for f in mrqa_features], dtype=torch.long)
            train_features = mrqa_features
            ...
            #print(len(all_input_type), len(all_input_ids), len(all_input_mask), len(all_segment_ids), len(all_start_positions), len(all_end_positions))
        ...
        if args.debug=="True":
            train_data = Subset(train_data, np.arange(100))
    ...
    if args.do_predict:
        ...
        if args.predict_squad:
            ...
            #metrics = evaluate_squadv2(args.predict_file, output_prediction_file) 
            #print(metrics)
            """ commented part: Included in the shell script"""
            #with open(os.path.join(export_root, args.output_name+'_prediction_metrics.json'), 'w') as f:
            #   json.dump(metrics, f, indent=4)
